...
// EOF - Tomcraft - 2009-11-28 - Included xs:booster
      ...
      if (isset ($_POST['multi_g_taxonomie_confirm'])) {
        ...
        {
          if (xtc_not_null($_POST['google_multi_category_'.$langdata['id']]))
          {
            $dest_google_category = xtc_db_prepare_input($_POST['google_multi_category_'.$langdata['id']]);
            if (!empty($_POST['multi_products']))
            {
              foreach ($_POST['multi_products'] AS $product_id) 
              {
                $q = xtc_db_query("SELECT COUNT(*) as count FROM `".TABLE_AGI_PRODUCTS_DESCRIPTION_ADD."` WHERE `products_id`='".xtc_db_input($product_id)."' AND `language_id`=".$langdata['id']);
                $q_count = xtc_db_fetch_array($q);
                if ($q_count['count'])
                  xtc_db_perform(TABLE_AGI_PRODUCTS_DESCRIPTION_ADD, array('google_category'=>$dest_google_category), 'update', "`products_id`='".xtc_db_input($product_id)."' AND `language_id`=".$langdata['id']);
                else
                  xtc_db_perform(TABLE_AGI_PRODUCTS_DESCRIPTION_ADD, array('products_id'=>xtc_db_input($product_id), 'language_id'=>$langdata['id'], 'google_category'=>$dest_google_category));
              }
            }
            if (!empty($_POST['multi_categories']))
            {
              foreach ($_POST['multi_categories'] AS $category_id) 
              {
                $q = xtc_db_query("SELECT COUNT(*) as count FROM `".TABLE_AGI_CATEGORIES_DESCRIPTION_ADD."` WHERE `categories_id`='".xtc_db_input($category_id)."' AND `language_id`=".$langdata['id']);
                $q_count = xtc_db_fetch_array($q);
                if ($q_count['count'])
                  xtc_db_perform(TABLE_AGI_CATEGORIES_DESCRIPTION_ADD, array('google_category'=>$dest_google_category), 'update', "`categories_id`='".xtc_db_input($category_id)."' AND `language_id`=".$langdata['id']);
                else
                  xtc_db_perform(TABLE_AGI_CATEGORIES_DESCRIPTION_ADD, array('categories_id'=>xtc_db_input($category_id), 'language_id'=>$langdata['id'], 'google_category'=>$dest_google_category));
              }
            }
          }
        }
