...
/* --------------------------------------------------------------
  ...
  $show_agi_google_taxonomy = false; // AGI GOOGLE-XML-EXPORT www.andreas-guder.de 1/5
            ...
            case 'multi_action':
              ...
              if (xtc_not_null($_POST['multi_g_taxonomie'])) {
                  ...
                  if ($agi_count)
                  {
                    ...
                    {
                      $contents[] = array('text' => xtc_image(DIR_WS_LANGUAGES . $langdata['directory'] .'/admin/images/'. $langdata['image'], $langdata['name']) . ' ' .xtc_draw_input_field('google_multi_category_'.$langdata['id'], '', 'readonly="readonly" style="width: 200px; background-color: #eee;"'));
                    }
...
?>
            <li <?php echo $arrayKey == 0 ? 'class="active"' : ''; ?>>
              <?php echo  xtc_draw_input_field('google_category_'.$langdata['id'], '', 'size="50" style="width: 800px" onChange="copy_google_category(this.value, '.$langdata['id'].');"'); ?><br />
              <br />
              <div id="g_taxonomie_select_<?php echo $langdata['id']; ?>" class="g_taxonomie_select"></div>
...
<?php } ?>
      ...
      <script type="text/javascript">
      /* <![CDATA[ */
        var target_elements = new Array('google_category', 'google_multi_category');
        function copy_google_category(value, langID)
        {
          document.getElementsByName('google_multi_category_'.langID)[0].value = value;
        }
      /* ]]> */
      </script>
    </td>
