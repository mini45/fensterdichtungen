...
$agi_options = $catfunc->agi_load_products_attributes($pInfo->products_id, $_SESSION['languages_id']);
...
if ($_GET['action'] == 'new_product') {
  ...
  echo '<tr><td colspan="3">'.xtc_draw_separator('pixel_trans.gif', '1', '10').'</td></tr>';
  echo '<tr>';
  if ($pInfo->products_image) 
  {
    echo '<td class="main" align="center" width="'. (PRODUCT_IMAGE_THUMBNAIL_WIDTH + 15).'">';
    echo xtc_image(DIR_WS_CATALOG_THUMBNAIL_IMAGES.$pInfo->products_image, 'Standard Image').'<br />';
    echo xtc_draw_selection_field('del_pic', 'checkbox', $pInfo->products_image).' '.TEXT_DELETE.'</td>';
  ...
  echo '<td class="main">'.TEXT_PRODUCTS_IMAGE.'<br />'.xtc_draw_file_field('products_image').'<br />'.xtc_draw_separator('pixel_trans.gif', '24', '15').'&nbsp;'.$pInfo->products_image.xtc_draw_hidden_field('products_previous_image_0', $pInfo->products_image).'</td>';
  echo '<td class="main">'.$catfunc->agi_get_products_image_attribute_selection($agi_options, $pInfo->products_id, 0).'</td>';
  echo '</tr>';
  ...
  if (MO_PICS > 0) {
    ...
    for ($i = 0; $i < MO_PICS; $i ++) 
    {
      echo '<tr><td colspan="3">'.xtc_draw_separator('pixel_black.gif', '100%', '1').'</td></tr>';
      echo '<tr><td colspan="3">'.xtc_draw_separator('pixel_trans.gif', '1', '10').'</td></tr>';
      if (isset ($mo_images[$i]["image_name"]))
      {
        echo '<td class="main" align="center" width="'. (PRODUCT_IMAGE_THUMBNAIL_WIDTH + 15).'">';
        echo xtc_image(DIR_WS_CATALOG_THUMBNAIL_IMAGES.$mo_images[$i]["image_name"], 'Image '. ($i +1)).'<br />';
        echo xtc_draw_selection_field('del_mo_pic[]', 'checkbox', $mo_images[$i]["image_name"]).' '.TEXT_DELETE.'</td>';
      }
      else
      {
        echo '<td></td>';
      }
      echo '<td class="main">'.TEXT_PRODUCTS_IMAGE.' '. ($i +1).'<br />'.xtc_draw_file_field('mo_pics_'.$i).'<br />'.xtc_draw_separator('pixel_trans.gif', '24', '15').'&nbsp;'.$mo_images[$i]["image_name"].xtc_draw_hidden_field('products_previous_image_'. ($i +1), $mo_images[$i]["image_name"]).'</td>';
      echo '<td class="main">'.$catfunc->agi_get_products_image_attribute_selection($agi_options, $pInfo->products_id, ($i +1)).'</td>';
      echo '</tr>';
    }
  }
