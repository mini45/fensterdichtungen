...
<?php
  $agi_google_cats = array();
  if (!empty($cInfo->categories_id))
  {
    $q = xtc_db_query("SELECT `language_id`, `google_category` FROM `".TABLE_AGI_CATEGORIES_DESCRIPTION_ADD."` WHERE `categories_id`=".$cInfo->categories_id);
    while ($qdata = xtc_db_fetch_array($q))
      $agi_google_cats[$qdata['language_id']] = $qdata['google_category'];
  }
  foreach ($languages as $arrayKey => $langdata) 
  {
?>
                <li <?php echo $arrayKey == 0 ? 'class="active"' : ''; ?>>
                  <?php echo  xtc_draw_input_field('google_category_'.$langdata['id'], !empty($agi_google_cats[$langdata['id']]) ? $agi_google_cats[$langdata['id']] : '', 'size="50" style="width: 96%"'); ?><br />
                  <br />
