...
class categories {
  ...
  function remove_category($category_id) {
    ...
    /* BOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 1/7 */
    if (defined('MODULE_AGI_GOOGLE_XML_VERSION'))
      xtc_db_query("DELETE FROM ".TABLE_AGI_CATEGORIES_DESCRIPTION_ADD." WHERE categories_id = '".xtc_db_input($category_id)."'");
    /* EOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 1/7 */
  ...
  function insert_category($categories_data, $dest_category_id, $action = 'insert') {
    ...
    /* BOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 2/7 */
    $this->agi_categories_description_add($categories_id, $categories_data);
    /* EOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 2/7 */
  ...
  */
  ...
  function copy_category($src_category_id, $dest_category_id, $ctype = "link") {
    ...
    if (!(in_array($src_category_id, $_SESSION['copied']))) {
      ...
      /* BOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 3/7 */
      // copy AGI additional_values
      if (defined('MODULE_AGI_GOOGLE_XML_VERSION')) // AGI GOOGLE-XML-EXPORT www.andreas-guder.de
      {
        $cdcopy_query = xtc_db_query("SELECT * FROM ".TABLE_AGI_CATEGORIES_DESCRIPTION_ADD." WHERE categories_id = '".$src_category_id."'");
        while ($cdcopy_values = xtc_db_fetch_array($cdcopy_query)) {
          $sql_data_array = $cdcopy_values;
          //set new descriptions (overrides)
          $sql_data_array['categories_id'] = $new_cat_id;
          //write descriptions to DB
          xtc_db_perform(TABLE_AGI_CATEGORIES_DESCRIPTION_ADD, $sql_data_array);
        }
      }
      /* EOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 3/7 */
    }
  }
  ...
  function remove_product($product_id) {
    ...
    /* BOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 4/7 */
    if (defined('MODULE_AGI_GOOGLE_XML_VERSION'))
    {
      xtc_db_query("DELETE FROM `".TABLE_AGI_PRODUCTS_ADD."` WHERE `products_id` = '".xtc_db_input($product_id)."'");
      xtc_db_query("DELETE FROM `".TABLE_AGI_PRODUCTS_DESCRIPTION_ADD."` WHERE `products_id` = '".xtc_db_input($product_id)."'");
      xtc_db_query("DELETE FROM `".TABLE_AGI_PRODUCTS_IMAGES_TO_ATTRIBUTES."` WHERE `products_id` = '".xtc_db_input($product_id)."'");
    }
    /* EOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 4/7 */
    if (USE_CACHE == 'true') {
      xtc_reset_cache_block('categories');
      xtc_reset_cache_block('also_purchased');
    }
  ...
  function insert_product($products_data, $dest_category_id, $action = 'insert') {
    ...
    /* BOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 5/7 */
    $this->agi_products_data_add($products_id, $products_data);
    /* EOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 5/7 */
  ...
  function duplicate_product($src_products_id, $dest_categories_id) {
    ...
    /* BOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 6/7 */
    // copy AGI additional_values
    if (defined('MODULE_AGI_GOOGLE_XML_VERSION'))
    {
      $cdcopy_query = xtc_db_query("SELECT * FROM ".TABLE_AGI_PRODUCTS_ADD." WHERE products_id = '".$old_products_id."'");
      while ($cdcopy_values = xtc_db_fetch_array($cdcopy_query)) {
        $sql_data_array = $cdcopy_values;
        $sql_data_array['products_id'] = $dup_products_id;
        //write descriptions to DB
        xtc_db_perform(TABLE_AGI_PRODUCTS_ADD, $sql_data_array);
      }
      $cdcopy_query = xtc_db_query("SELECT * FROM ".TABLE_AGI_PRODUCTS_DESCRIPTION_ADD." WHERE products_id = '".$old_products_id."'");
      while ($cdcopy_values = xtc_db_fetch_array($cdcopy_query)) {
        $sql_data_array = $cdcopy_values;
        $sql_data_array['products_id'] = $dup_products_id;
        //write descriptions to DB
        xtc_db_perform(TABLE_AGI_PRODUCTS_DESCRIPTION_ADD, $sql_data_array);
      }
    }
    /* EOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 6/7 */
  ...
  /* BOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 7/7 */
  function agi_categories_description_add($categories_id, $categories_data)
  {
    if (!defined('MODULE_AGI_GOOGLE_XML_VERSION'))
      return;
    ...
    {
      $dest_google_category = xtc_db_prepare_input($categories_data['google_category_'.$langdata['id']]);
      $q = xtc_db_query("SELECT COUNT(*) as count FROM `".TABLE_AGI_CATEGORIES_DESCRIPTION_ADD."` WHERE `categories_id`='".xtc_db_input($categories_id)."' AND `language_id`=".$langdata['id']);
      $q_count = xtc_db_fetch_array($q);
      if ($q_count['count'])
        xtc_db_perform(TABLE_AGI_CATEGORIES_DESCRIPTION_ADD, array('google_category'=>$dest_google_category), 'update', "`categories_id`='".xtc_db_input($categories_id)."' AND `language_id`=".$langdata['id']);
      else
      {
        $insert = array(
          'categories_id'=> xtc_db_input($categories_id),
          'language_id'  => $langdata['id'],
          'google_category'=> $dest_google_category
        );
        xtc_db_perform(TABLE_AGI_CATEGORIES_DESCRIPTION_ADD, $insert);
      }
    }
  }
  
  function agi_products_data_add($products_id, $products_data)
  {
    $languages = xtc_get_languages();
    $products_add = array(
      'products_id'                 => $products_id,
      'google_export'               => array_key_exists('agi_google_export', $products_data) ? 1 : 0,
      'google_condition'            => xtc_db_prepare_input($products_data['agi_google_condition']),
      'google_excluded_destination' => xtc_db_prepare_input($products_data['agi_google_excluded_destination']),
      'google_expiration_date'      => xtc_db_prepare_input($products_data['agi_google_expiration_date']),
      'google_products_brand'       => xtc_db_prepare_input($products_data['agi_google_products_brand']),
      'google_identifier_exists'    => xtc_db_prepare_input($products_data['agi_google_identifier_exists']),
      'google_multipack_amount'     => xtc_db_prepare_input($products_data['agi_google_multipack_amount']),
      'google_energy_efficiency'    => xtc_db_prepare_input($products_data['agi_google_energy_efficiency']),
      'clothing_product'            => array_key_exists('clothing_product', $products_data) ? 1 : 0,
      'google_attribute_gender'     => xtc_db_prepare_input($products_data['agi_google_attribute_gender']),
      'google_attribute_age_group'  => xtc_db_prepare_input($products_data['agi_google_attribute_age_group']),
      'google_attribute_size_type'  => xtc_db_prepare_input($products_data['agi_google_attribute_size_type']),
      'google_attribute_size_system'  => xtc_db_prepare_input($products_data['agi_google_attribute_size_system'])
    );
    
    $q = xtc_db_query("SELECT COUNT(*) as count FROM `".TABLE_AGI_PRODUCTS_ADD."` WHERE `products_id`='".xtc_db_input($products_id)."'");
    $q_count = xtc_db_fetch_array($q);
    if ($q_count['count'])
      xtc_db_perform(TABLE_AGI_PRODUCTS_ADD, $products_add, 'update', "`products_id`='".xtc_db_input($products_id)."'");
    else
      xtc_db_perform(TABLE_AGI_PRODUCTS_ADD, $products_add);
    
    foreach ($languages as $langdata) 
    {
      $products_desc_add = array(
        'products_id'               => $products_id,
        'language_id'               => $langdata['id'],
        'google_category'           => xtc_db_prepare_input($products_data['google_category_'.$langdata['id']]),
        'google_adwords_redirect'   => xtc_db_prepare_input($products_data['agi_google_adwords_redirect_'.$langdata['id']]),
        'google_mobile_link'        => xtc_db_prepare_input($products_data['agi_google_mobile_link_'.$langdata['id']]),
        'google_alternative_title'  => xtc_db_prepare_input($products_data['agi_google_alternative_title_'.$langdata['id']]),
        'google_adwords_grouping'   => xtc_db_prepare_input($products_data['agi_google_adwords_grouping_'.$langdata['id']]),
        'google_adwords_labels'     => xtc_db_prepare_input($products_data['agi_google_adwords_labels_'.$langdata['id']]),
        'google_attribute_color'    => xtc_db_prepare_input($products_data['agi_google_attribute_color_'.$langdata['id']]),
        'google_attribute_size'     => xtc_db_prepare_input($products_data['agi_google_attribute_size_'.$langdata['id']]),
        'google_attribute_pattern'  => xtc_db_prepare_input($products_data['agi_google_attribute_pattern_'.$langdata['id']]),
        'google_attribute_material' => xtc_db_prepare_input($products_data['agi_google_attribute_material_'.$langdata['id']]),
        'google_custom_label0'      => xtc_db_prepare_input($products_data['agi_google_custom_label0_'.$langdata['id']]),
        'google_custom_label1'      => xtc_db_prepare_input($products_data['agi_google_custom_label1_'.$langdata['id']]),
        'google_custom_label2'      => xtc_db_prepare_input($products_data['agi_google_custom_label2_'.$langdata['id']]),
        'google_custom_label3'      => xtc_db_prepare_input($products_data['agi_google_custom_label3_'.$langdata['id']]),
        'google_custom_label4'      => xtc_db_prepare_input($products_data['agi_google_custom_label4_'.$langdata['id']])
      );
      
      $q = xtc_db_query("SELECT COUNT(*) as count FROM `".TABLE_AGI_PRODUCTS_DESCRIPTION_ADD."` WHERE `products_id`='".xtc_db_input($products_id)."' AND `language_id`=".$langdata['id']);
      $q_count = xtc_db_fetch_array($q);
      if ($q_count['count'])
        xtc_db_perform(TABLE_AGI_PRODUCTS_DESCRIPTION_ADD, $products_desc_add, 'update', "`products_id`='".xtc_db_input($products_id)."' AND `language_id`=".$langdata['id']);
      else
        xtc_db_perform(TABLE_AGI_PRODUCTS_DESCRIPTION_ADD, $products_desc_add);
    }
    $this->agi_prepare_attribute_images($products_id, $products_data);
  }
  
  function agi_prepare_attribute_images($products_id, $products_data)
  {
    // delete all attribute_images
    xtc_db_query("DELETE FROM `".TABLE_AGI_PRODUCTS_IMAGES_TO_ATTRIBUTES."` WHERE `products_id`=".(int)$products_id);
     
    // load actually image
    $q = "SELECT IFNULL(`products_image`,'') AS products_image FROM `".TABLE_PRODUCTS."` WHERE `products_id`='".xtc_db_input($products_id)."'";
    $q_result = xtc_db_query($q);
    $q_data = xtc_db_fetch_array($q_result);
    if (!empty($q_data['products_image']) && array_key_exists('agi_image_attribute_0', $products_data))
      $this->agi_save_attribute_images($products_data['agi_image_attribute_0'], $products_id, 0);
    
    if (defined('TABLE_PRODUCTS_IMAGES') && defined('MO_PICS'))
    {
      $q = "SELECT `image_nr` FROM `" . TABLE_PRODUCTS_IMAGES . "` WHERE `products_id` = ".(int)$products_id." ORDER BY image_nr";
      $q_result = xtc_db_query($q);
      while ($q_data = xtc_db_fetch_array($q_result))
      {
        if (array_key_exists('agi_image_attribute_'.$q_data['image_nr'], $products_data))
          $this->agi_save_attribute_images($products_data['agi_image_attribute_'.$q_data['image_nr']], $products_id, $q_data['image_nr']);
      }
    }
  }
  
  function agi_save_attribute_images($products_image_data, $products_id, $image_id)
  {
    if (is_array($products_image_data))
    {
      foreach ($products_image_data as $value)
      {
        if (!empty($value))
        {
          $data = explode(':',$value);
          if (count($data) == 2 && !empty($value[0]))
          {
            $sql_data = array('image_nb' => $image_id, 'products_id' => $products_id, 'options_id'=> $value[0], 'options_values_id' => $data[1] == 'all' ? 0 : $data[1]);
            xtc_db_perform(TABLE_AGI_PRODUCTS_IMAGES_TO_ATTRIBUTES, $sql_data);
          }
        }
      }
    }
  ...
  function agi_load_products_attributes($products_id, $language_id)
  {
    $agi_shown_options = array();
    if (empty($products_id))
      return array();
      
    $q = "SELECT `options_id`, `options_values_id` FROM `".TABLE_PRODUCTS_ATTRIBUTES."` WHERE `products_id`=".(int)$products_id." ORDER BY `sortorder`";
    $q_result = xtc_db_query($q);
    $tmp_shown_options = array();
    $tmp_shown_options_values = array();
    $agi_product_attributes = array();
    while ($row = xtc_db_fetch_array($q_result))
    {
      $tmp_shown_options[] = $row['options_id'];
      $tmp_shown_options_values[] = $row['options_values_id'];
      $agi_product_attributes[] = array('options_id'=> $row['options_id'], 'options_values_id'=> $row['options_values_id']);
    }
    if (!empty($agi_product_attributes))
    {

      $q = "SELECT `products_options_id`, `products_options_name` FROM `".TABLE_PRODUCTS_OPTIONS."` WHERE `products_options_id` IN (".implode(',',$tmp_shown_options).") AND `language_id`=".(int)$language_id." ORDER BY `products_options_sortorder`";
      $q_result = xtc_db_query($q);
      $agi_shown_options = array();
      while($row = xtc_db_fetch_array($q_result))
      {
        $agi_shown_options[$row['products_options_id']] = array('options_id'=>$row['products_options_id'], 'name'=>$row['products_options_name'], 'values'=> array());
        $agi_shown_options[$row['products_options_id']]['values'][] = array('id' => '', 'text'=>AGI_NO_IMAGE_ATTRIBUTES);
        $agi_shown_options[$row['products_options_id']]['values'][] = array('id' => $row['products_options_id'].':all', 'text'=>AGI_ALL_IMAGE_ATTRIBUTES);
      }
      if (empty($agi_shown_options))
      {
        $q = "SELECT MIN(`language_id`) AS minid FROM `".TABLE_PRODUCTS_OPTIONS."`";
        $q_result = xtc_db_fetch_array(xtc_db_query($q));
        $language_id = $q_result['minid'];
        ...
        while($row = xtc_db_fetch_array($q_result))
        {
          $agi_shown_options[$key] = array('options_id'=>$row['products_options_id'], 'name'=>$row['products_options_name'], 'values'=> array());
          $agi_shown_options[$key]['values'][] = array('id' => '', 'text'=>AGI_NO_IMAGE_ATTRIBUTES);
          $agi_shown_options[$key]['values'][] = array('id' => $row['products_options_id'].':all', 'text'=>AGI_ALL_IMAGE_ATTRIBUTES);
        }
      }

      $q = "SELECT `products_options_values_id`, `products_options_values_name` FROM `".TABLE_PRODUCTS_OPTIONS_VALUES."` WHERE `products_options_values_id` IN (".implode(',',$tmp_shown_options_values).") AND `language_id`=".(int)$language_id;
      $q_result = xtc_db_query($q);
      $agi_option_values = array();
      while($row = xtc_db_fetch_array($q_result))
      {
        $agi_option_values[$row['products_options_values_id']] = $row['products_options_values_name'];
      }
      foreach ($agi_product_attributes as $row)
      {
        if (array_key_exists($row['options_id'], $agi_shown_options) && array_key_exists($row['options_values_id'], $agi_option_values))
        {
          $agi_shown_options[$row['options_id']]['values'][] = array('id'=>$row['options_id'].':'.$row['options_values_id'], 'text'=>$agi_option_values[$row['options_values_id']]);
        }
      }
    }
    return $agi_shown_options;
  }
  
  function agi_get_products_image_attribute_selection($products_attributes, $products_id = 0, $image_id = 0)
  {
    if (empty($products_attributes))
      return '';
    $image_options = array();
    $q = "SELECT * FROM `".TABLE_AGI_PRODUCTS_IMAGES_TO_ATTRIBUTES."` WHERE `products_id`=".(int)$products_id." AND `image_nb`=".(int)$image_id;
    $q_result = xtc_db_query($q);
    while ($row = xtc_db_fetch_array($q_result))
    {
      $image_options[$row['options_id']] = empty($row['options_values_id']) ? $row['options_id'].':all' : $row['options_id'].':'.$row['options_values_id'];
    }
    
    $output = '<table summary="Attribute zum Bild" border="0"><tbody>';
    foreach ($products_attributes as $option_id => $option_data)
    {
      $output .= '<tr><td class="main">';
      $output .= htmlentities($option_data['name'], ENT_COMPAT, $_SESSION['language_charset']);
      $output .= '</td><td>';
      $output .= xtc_draw_pull_down_menu('agi_image_attribute_'.$image_id.'[]', $option_data['values'], array_key_exists($option_id, $image_options) ? $image_options[$option_id] : '');
      $output .= '</td></tr>';
    }
    $output .= '</tbody></table>';
    return $output;
  }
  /* EOF AGI GOOGLE-XML-EXPORT www.andreas-guder.de 7/7 */
