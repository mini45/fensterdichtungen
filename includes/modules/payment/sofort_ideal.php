<?php
/**
 * @version SOFORT iDEAL - $Date: 2013-04-22 14:00:13 +0200 (Mon, 22 Apr 2013) $
...
class sofort_ideal {
	...
	function sofort_ideal () {
		...
		if (defined('MODULE_PAYMENT_SOFORT_IDEAL_CLASSIC_RECOMMENDED_PAYMENT') && MODULE_PAYMENT_SOFORT_IDEAL_CLASSIC_RECOMMENDED_PAYMENT == 'True') $this->title_extern .= ' '.MODULE_PAYMENT_SOFORT_IDEAL_CLASSIC_RECOMMENDED_PAYMENT_TEXT;
		...
		if ((int) MODULE_PAYMENT_SOFORT_IDEAL_CLASSIC_ORDER_STATUS_ID > 0) $this->order_status = MODULE_PAYMENT_SOFORT_IDEAL_CLASSIC_ORDER_STATUS_ID;
	...
	function selection () {
		...
		switch (MODULE_PAYMENT_SOFORT_IDEAL_CLASSIC_IMAGE) {
			case 'Logo & Text': $description .= $this->setImageText('logo_155x50.png', MODULE_PAYMENT_SOFORT_IDEAL_CLASSIC_TEXT_DESCRIPTION_CHECKOUT_PAYMENT_TEXT); break;
			case 'Infographic':
			default:			$description .= $this->setImageText('banner_300x100.png', ''); break;
		}
		...
		if(!empty($banks) && is_array($banks)) {
			foreach ($banks as $value) {
				...
				if( isset( $_SESSION[ 'ideal_bank_name' ] ) && $_SESSION['ideal_bank_name'] == $value['code'] ) $selectBox .= ' selected="selected"';
	...
	function pre_confirmation_check ($vars = '') {
		// Fix for XTC Bug
		if (empty($_SESSION['cart']->cartID)) $_SESSION['cart']->cartID = $_SESSION['cart']->generate_cart_id();
		...
		if(isset($requestData['ideal_bank_name'])) $_SESSION['ideal_bank_name'] = HelperFunctions::htmlMask($requestData['ideal_bank_name']);
	...
	function after_process () {
		...
		if (isset($_SESSION['ideal_bank_name'])) unset($_SESSION['ideal_bank_name']);
	}
	...
	function get_error () {
		if (isset($_GET['payment_error']) && $_GET['payment_error'] != 'sofort_ideal') return false;
	...
	function _checkExistingSofortConstants() {
		...
		if(defined('MODULE_PAYMENT_SOFORT_IDEAL_TEXT_TITLE')) return true;
		
		$lngdir = DIR_FS_CATALOG.'lang/';
		$iterator = new DirectoryIterator($lngdir);
		
		foreach ($iterator as $file) if ($file->isDir() && file_exists($lngdir.$file->getFilename().'/modules/payment/sofort_general.php')) $installedModulLangs[] = $file->getFilename();
		
		//currently installed in this module
		if(!in_array($_SESSION['language'], $installedModulLangs)) include_once('lang/english/modules/payment/sofort_ideal.php');
	...
	function _insertAndReturnSofortStatus() {
		...
		foreach ($iterator as $file) if ($file->isDir() && file_exists($lngdir.$file->getFilename().'/modules/payment/sofort_general.php')) $sofortLangs[] = strtoupper($file->getFilename());
		...
		foreach($installedLangs as $installedLang) {
			...
			if($sofortStatuses['ideal_temp'] === false || $sofortStatuses['ideal_temp'] == '') $sofortStatuses['ideal_temp'] = $this->_insertStatusInDb($newOrdersStatusName, $ordersStatusIdTemp, $langId);
			...
			if($sofortStatuses['ideal_confirmed'] === false || $sofortStatuses['confirmed'] == '') $sofortStatuses['ideal_confirmed'] = $this->_insertStatusInDb($newOrdersStatusName, $ordersStatusIdConfirmed, $langId);
			...
			if($sofortStatuses['ideal_aborted'] === false || $sofortStatuses['ideal_aborted'] == '') $sofortStatuses['ideal_aborted'] = $this->_insertStatusInDb($newOrdersStatusName, $ordersStatusIdAborted, $langId);
			...
			if($sofortStatuses['ideal_loss'] === false || $sofortStatuses['ideal_loss'] == '') $sofortStatuses['ideal_loss'] = $this->_insertStatusInDb($newOrdersStatusName, $ordersStatusIdLoss, $langId);
			...
			if($sofortStatuses['ideal_ref_com'] === false || $sofortStatuses['ideal_ref_com'] == '') $sofortStatuses['ideal_ref_com'] = $this->_insertStatusInDb($newOrdersStatusName, $ordersStatusIdRefCom, $langId);
			...
			if($sofortStatuses['ideal_ref_ref'] === false || $sofortStatuses['ideal_ref_ref'] == '') $sofortStatuses['ideal_ref_ref'] = $this->_insertStatusInDb($newOrdersStatusName, $ordersStatusIdRefRef, $langId);
		}
