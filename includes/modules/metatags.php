...
// ---------------------------------------------------------------------------------------
  ...
  $metaMinLength   =  5;     // Mindestl?nge eines Keywords
  ...
  case FILENAME_PRODUCT_INFO :
    ...
    if($product->isProduct()) {
      ...
      if(!empty($product->data['products_meta_keywords'])) {
        $keywords = $product->data['products_meta_keywords'];
        if(strpos($keywords,'canonical:') !== false){
          $exploded = explode(',',$keywords);
          $canon = $exploded[0];

          $realCanon = str_replace('canonical:','',$canon);
          
          $keywords = str_replace($exploded[0],'',$keywords);
...
//        $canonical_url = $realCanon;
        $canonical_url = xtc_href_link(FILENAME_PRODUCT_INFO, 'products_id='.$realCanon,$request_type,false);
      }
...
*/
// 06.01.2014 Pr?fen, ob Bootstrap als Template gew?hlt ist und Variable html5 setzen
...
if (isset($html5)) { echo '<meta name="viewport" content="width=device-width, initial-scale=1.0">'."\n"; }
if (!isset($html5)) {
