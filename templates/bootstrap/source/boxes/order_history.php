  ...
  if (isset($_SESSION['customer_id'])) {
    ...
    if (xtc_db_num_rows($orders_query)) {
      ...
      while ($products = xtc_db_fetch_array($products_query)) {
        $customer_orders_string .= '<div class="row-fluid">' .
                                   '    <div class="span12"><a href="' . xtc_href_link(FILENAME_PRODUCT_INFO, xtc_product_link($products['products_id'],$products['products_name'])) . '">' . $products['products_name'] . '</a>' .
                                   '    <a class="pull-right" href="' . xtc_href_link(basename($PHP_SELF), xtc_get_all_get_params(array('action')) . 'action=cust_order&pid=' . $products['products_id']) . '" title="'.BOX_HEADING_ADD_PRODUCT_ID.'"><i class="icon-shopping-cart"></i></a></div>' .
                                   '</div>';
      }
      $customer_orders_string .= '';
