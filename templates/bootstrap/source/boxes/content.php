...
if (!$box_smarty->is_cached(CURRENT_TEMPLATE.'/boxes/box_content.php', $cache_id) || !$cache) {
	...
	while ($content_data = xtc_db_fetch_array($content_query, true)) {
		...
		$data[] = '<a href="'.xtc_href_link(FILENAME_CONTENT, 'coID='.$content_data['content_group'].$SEF_parameter).'">'.$content_data['content_title'].'</a>';
	}
	$box_smarty->assign('BOX_CONTENT', $data);
	//if ($content_string != '') {
		//$content_string.='</ul>';
	//	$box_smarty->assign('BOX_CONTENT', $data);
	//}
...
if (!$cache) {
    $box_content = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_content.php');
} else {
    $box_content = $box_smarty->fetch(CURRENT_TEMPLATE.'/boxes/box_content.php', $cache_id);
}
