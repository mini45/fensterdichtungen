<?php
  ...
  function xtc_show_category($counter, $oldlevel=1) {
    ...
    if ($level > $oldlevel) { //neue Unterebene
      $ul = "\n" . $tab. '<ul class="nav nav-list dropdown">'. "\n";
...
//    if ($this_category == $counter) $cat_active = " active".$level;
    ...
    // Meine ƒnderung Zeichen f¸r Kategorie mit Unterkategorie einf¸gen
          if ($foo[$counter]['hassubcat']== true) {
          $IPre = '<b class="caret"></b>';
          }
